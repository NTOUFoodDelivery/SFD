...
public class OrderDaoImpl implements OrderDao {
  ...
  public boolean addOrder(Order order) {
    ...
    try {
      ...
      preparedStatement = connection.prepareStatement(order_sql);
      ...
      preparedStatement = connection.prepareStatement(OC_sql);
      preparedStatement.setLong(1, order.getOrder().getOrderID());
      ...
      preparedStatement = connection.prepareStatement(order_food_sql);  //
  ...
  public List<Order> searchIdleOrder() {
    ...
    try {
      String sql =
          "SELECT `order`.Order_Id, `order`.Total, `order`.Start_Time, `order`.Order_Status, `order`.Other,`order`.Type_Count, `order`.Address, `order`.Casting_Prio, customer_deliver_info.Customer_Id,member.User_Name,member.Account,member.Phone_Number\n"
              +
              "FROM `order`\n" +
  ...
  public List<Order> searchEaterOrder(Long userID) {
    ...
    try {
      ...
      String sql =
          "SELECT `order`.Order_Id, `order`.Total, `order`.Type_Count, `order`.Other, `order`.Address, `order`.Casting_Prio, member.User_Id, member.Account, member.User_Name, member.Phone_Number"
              +
              " FROM `order`" +
              "INNER JOIN customer_deliver_info ON `order`.Order_Id = customer_deliver_info.Order_Id"
              +
      ...
      while (resultSet.next()) {
        ...
        String mealSql =
            "SELECT order_food.Food_Id, order_food.`Count`, meal.Food_Name, meal.Cost, restaurant_info.Rest_Name, restaurant_info.Rest_Address"
                +
                " FROM order_food" +
                " INNER JOIN meal ON order_food.Food_Id = meal.Food_Id" +
                " INNER JOIN restaurant_info ON restaurant_info.Rest_Id = meal.Rest_Id" +
...
//  order_food.Order_SERIAL, order_food.`Count`, meal.Food_Name, restaurant_info.Rest_Name, customer_deliver_info.Customer_Id, customer_deliver_info.Deliver_Id " +
      ...
      if (resultSet.next()) {
        ...
        preparedStatement2 = (PreparedStatement) connection
            .prepareStatement(insertHO_CD_history_Id);
        preparedStatement2.setLong(1, orderID);
        preparedStatement2.executeUpdate();
        ...
        if (resultSet.next()) {
          ...
          String selectfoodsql =
              "SELECT  order_food.Order_SERIAL, order_food.Order_Id, order_food.`Count`, meal.Food_Name, restaurant_info.Rest_Name "
      ...
      C3P0Util.close(connection, resultSet, preparedStatement, preparedStatement2);
      return success;
    }
  }
  ...
  public List<Order> searchEaterHistoryOrder(Long userID) {
    ...
    try {
      ...
      while (resultSet.next()) {
        ...
        ResultSet mealResultSet;
        String mealSql =
            "SELECT history_food.Food_Name,history_food.Count, history_food.Rest_Name" + //加了NAME
                " FROM history" +
  ...
  public List<Order> searchDeliverOrder(Long userID) {
    ...
    try {
      String sql =
          "SELECT `order`.Order_Id, `order`.Total, `order`.Other, `order`.Address, member.Account, member.User_Name, member.Phone_Number "
              +
              " FROM `order`" +
              " INNER JOIN customer_deliver_info ON `order`.Order_Id = customer_deliver_info.Order_Id "
              +
      ...
      while (resultSet.next()) {
        ...
        String mealSql =
            "SELECT  order_food.`Count`, meal.Food_Name, meal.Cost, restaurant_info.Rest_Name, restaurant_info.Rest_Address"
                +
                " FROM order_food" +
                " INNER JOIN meal ON order_food.Food_Id = meal.Food_Id " +
                "INNER JOIN restaurant_info ON restaurant_info.Rest_Id = meal.Rest_Id " +
  ...
  public ArrayList<Order> searchallOrder() {
    ...
    try {
      String sql =
          "SELECT `order`.Order_Id, `order`.Total, `order`.Start_Time, `order`.Order_Status, `order`.Other,`order`.Type_Count, `order`.Address, `order`.Casting_Prio, customer_deliver_info.Customer_Id,customer_deliver_info.Deliver_Id, member.User_Name,member.Account,member.Phone_Number\n"
              +
              " FROM `order`\n" +
              " INNER JOIN customer_deliver_info ON `order`.Order_Id = customer_deliver_info.Order_Id\n"
              +
      ...
      String sql_del =
          "SELECT customer_deliver_info.Deliver_Id, member.User_Name,member.Account,member.Phone_Number"
      ...
      C3P0Util.close(resultSet, rs);
      C3P0Util.close(preparedStatement, pst);
  ...
  public List<Order> searchallHistoryOrder() {
    ...
    try {
      String sql =
          "SELECT history.History_Id, history.Start_Time, history.Type_Count, history.Total, history.Final_Status, history.Address, history.Other,history_customer_deliver_info.Customer_Id, member.User_Name,member.Account,member.Phone_Number"
              + " FROM history"
              + " INNER JOIN history_customer_deliver_info ON history.History_Id = history_customer_deliver_info.History_Id\n"
              + " INNER JOIN member ON  `member`.User_Id = history_customer_deliver_info.Customer_Id";
      String sql_del =
          "SELECT history_customer_deliver_info.Deliver_Id, member.User_Name,member.User_Name,member.Account,member.Phone_Number"
      ...
      while (resultSet.next() && rs.next()) {
        ...
        ResultSet mealResultSet;
        String mealSql =
            "SELECT History_food.`Count`, History_food.Food_Name, History_food.Rest_Name, History_food.Address" +
                " FROM history_food" +
        ...
        while (mealResultSet.next()) {
          ...
          mealsBean.setRestName(mealResultSet.getString("Rest_Name"));
          mealsBean.setRestAddress(mealResultSet.getString("Address"));
          mealsBean.setFoodName(mealResultSet.getString("Food_Name"));
          mealsBean.setCount(mealResultSet.getInt("Count"));
          mealsBeanXList.add(mealsBean);
        }
